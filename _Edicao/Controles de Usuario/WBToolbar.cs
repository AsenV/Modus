...
namespace Modus
{
    /* no construtor: Application.AddMessageFilter(new ToolbarMessageFilter(wbToolBar1)); // mousedown unfocus
     * Adicione esse metodo ao formulario:
    private void InitializeWBToolBar()
    {
        // Adicione botões à barra de ferramentas (use "-" como separador)
        wbToolBar1.AddButton("Arquivo", new List<string> { "Novo", "Abrir", "Salvar", "-", "Fechar" }, index =>
        {
            if (index == 0) { }
            else if (index == 1) { }
            else if (index == 2) { }
        ...
        wbToolBar1.AddButton("Sobre", new List<string> { "Dicas", "-", "Sobre" }, index =>
        { });
    }
    */
    public partial class WBToolBar : UserControl
    {
        ...
        private readonly List<WBToolBarButton> _buttons = new();
        private MetroContextMenu contextMenu;
        private WBToolBarButton _clickedButton;
        private WBToolBarButton _lastClickedButton;
        ...
        private void SetupButtonAppearance(WBToolBarButton button)
        {
        ...
        {
            var button = new WBToolBarButton(buttonText, menuItemTexts, menuItemClicked);
        ...
        private void SetupButtonEvents(WBToolBarButton button, Action<int> menuItemClicked)
        {
            var btn = button.Button;
        ...
        private List<ToolStripItem> CreateToolStripItems(WBToolBarButton button)
        {
        ...
        public class WBToolBarButton
        {
            ...
            public WBToolBarButton(string text, IEnumerable<string> menuItemTexts, Action<int> menuItemClicked)
            {
        ...
        {
            int lines = flowLayoutPanel1.Height / 30;
            this.Height = lines > 1 ? flowLayoutPanel1.Height : 30;
        }
    }

    public class WBToolbarMessageFilter : IMessageFilter
    {
        private readonly WBToolBar wbToolBar;

        public WBToolbarMessageFilter(WBToolBar wbToolBar)
        {
        ...
        public bool PreFilterMessage(ref Message m)
        {
            ...
            {
                if (!wbToolBar.RectangleToScreen(wbToolBar.ClientRectangle).Contains(Cursor.Position))
                {
                    // Verifica se o mouse está dentro do menu de contexto antes de fechá-lo
                    if (wbToolBar.ContextMenuStrip != null && wbToolBar.ContextMenuStrip.Visible &&
                        wbToolBar.ContextMenuStrip.Bounds.Contains(Cursor.Position))
                    {
                        return true;
                    }
                    wbToolBar.HideContextMenu();
                }
            }
